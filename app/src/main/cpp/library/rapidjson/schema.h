...
private:
	...
	bool CreateParallelValidator(Context& context) const
	{
		...
		if (validatorCount_) {
			...
			if (not_) {
				context.validators[notValidatorIndex_] = context.factory.CreateSchemaValidator(
						*not_, false);
			}
			...
			const bool inheritContinueOnErrors) const
	{
		for (SizeType i = 0; i < schemas.count; i++)
			context.validators[schemas.begin + i] = context.factory.CreateSchemaValidator(
					*schemas.schemas[i], inheritContinueOnErrors);
	}
	...
	bool CheckUint(Context& context, uint64_t i) const
	{
		...
		if (!maximum_.IsNull()) {
			if (maximum_.IsUint64()) {
				...
				context.error_handler.AboveMaximum(i, maximum_, exclusiveMaximum_);
				RAPIDJSON_INVALID_KEYWORD_RETURN(
						exclusiveMaximum_ ? kValidateErrorExclusiveMaximum : kValidateErrorMaximum); // i >= 0 > maximum_
			}
			else if (!CheckDoubleMaximum(context, static_cast<double>(i))) {
				return false;
			}
		...
		if (v.IsObject()) {
			if (const SchemaType* sc = GetSchema(pointer)) {
				...
				SchemaType* s = new(allocator_->Malloc(sizeof(SchemaType))) SchemaType(this,
						pointer, v, document, allocator_, id);
				if (schema) {
					*schema = s;
				}
				return s->GetId();
			...
			if (len > 0) {
				...
				if (!base) {
					...
					if (len <= 1 || s[1] == '/') {
							...
							if (const ValueType* pv = relPointer.Get(*base)) {
								...
								if (pointer.IsValid() && !IsCyclicRef(pointer)) {
									...
									scopeId = pointer.GetUri(document, docId_,
											&unresolvedTokenIndex, allocator_);
						...
						if (const ValueType* pv = FindId(*base, ref, pointer,
							...
							if (!IsCyclicRef(pointer)) {
								...
								// TODO: cache pointer <-> id mapping
								size_t unresolvedTokenIndex;
								scopeId = pointer.GetUri(document, docId_, &unresolvedTokenIndex,
										allocator_);
								CreateSchema(schema, pointer, *pv, document, scopeId);
								return true;
							}
			...
			for (m = doc.MemberBegin(); m != doc.MemberEnd(); ++m) {
				if (m->value.GetType() == kObjectType || m->value.GetType() == kArrayType) {
					resval = FindId(m->value, finduri, resptr, localuri, full,
							here.Append(m->name.GetString(), m->name.GetStringLength(),
									allocator_));
				}
				if (resval) break;
			}
...
public:
	...
	*/
	...
	void EndMissingDependentProperties(const SValue& sourceName)
	{
		if (!missingDependents_.Empty()) {
			...
			AddErrorSchemaLocation(error,
					schemaRef.Append(sourceName.GetString(), sourceName.GetStringLength(),
							&GetInvalidSchemaPointer().GetAllocator()));
			ValueType wrapper(kObjectType);
			wrapper.AddMember(ValueType(SchemaType::GetValidateErrorKeyword(code),
					GetStateAllocator()).Move(), error, GetStateAllocator());
	...
	{
		AddErrorArray(matched ? kValidateErrorOneOfMatch : kValidateErrorOneOf, subvalidators,
				count);
	}
	void Disallowed()
	{
...
#undef RAPIDJSON_SCHEMA_HANDLE_VALUE_
			...
			const bool inheritContinueOnErrors)
	{
		ISchemaValidator* sv = new(
				GetStateAllocator().Malloc(sizeof(GenericSchemaValidator))) GenericSchemaValidator(
				*schemaDocument_, root, documentStack_.template Bottom<char>(),
...
private:
	...
	bool EndValue()
	{
		...
		*documentStack_.template Push<Ch>() = '\0';
		...
		uint64_t h = hasher
							 && CurrentContext().arrayUniqueness ? static_cast<HasherType*>(hasher)->GetHashCode() : 0;
			...
			if (hasher && context.valueUniqueness) {
				...
				if (!a) {
					CurrentContext().arrayElementHashCodes = a = new(
							GetStateAllocator().Malloc(sizeof(HashCodeArray))) HashCodeArray(
							kArrayType);
				}
	...
	void AddCurrentError(const ValidateErrorCode code, bool parent = false)
	{
		...
		AddError(ValueType(SchemaType::GetValidateErrorKeyword(code), GetStateAllocator(),
...
public:
	...
	*/
	SchemaValidatingReader(InputStream& is, const SchemaDocumentType& sd)
			: is_(is), sd_(sd), invalidSchemaKeyword_(), invalidSchemaCode_(kValidateErrorNone),
			  error_(kObjectType), isValid_(true) { }

	template<typename Handler>
	bool operator ()(Handler& handler)
	{
		GenericReader<SourceEncoding,
					  typename SchemaDocumentType::EncodingType,
					  StackAllocator> reader;
