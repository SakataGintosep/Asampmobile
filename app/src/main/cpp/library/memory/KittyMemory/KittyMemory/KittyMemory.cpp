...
bool memWrite(void* address, const void* buffer, size_t len, bool disableStupidGetProtection)
{
	...
	// Stupid getProtection workaround
	static std::map<void*, int> protMap{}; // Cache protection of address

	auto getStupidProtection = [](void* address) -> int {
		ProcMap addressMap = getAddressMap(address);
		if (!addressMap.isValid()) {
			return PROT_NONE;
		}

		return addressMap.protection;
	};

	int stupidProt = PROT_NONE;
	if (disableStupidGetProtection) {
		if (!protMap[address]) {
			stupidProt = getStupidProtection(address);
			if (stupidProt == PROT_NONE) {
				KITTY_LOGE("memWrite err couldn't find address (%p) in any map", address);
				return false;
			}

			protMap[address] = stupidProt;
		}

		stupidProt = protMap[address];
	}
	else {
		stupidProt = getStupidProtection(address);
		if (stupidProt == PROT_NONE) {
	...
	if (stupidProt & PROT_WRITE) {
		memcpy(address, buffer, len);
		return true;
	}

	if (setAddressProtection(address, len, stupidProt | PROT_WRITE) != 0) {
		KITTY_LOGE("memWrite err couldn't add write perm to address (%p, len: %zu, prot: %d)",
				address, len, stupidProt);
		return false;
	}

	memcpy(address, buffer, len);

	if (setAddressProtection(address, len, stupidProt) != 0) {
