...
void BitStream::WriteBits(const unsigned char* input, int numberOfBitsToWrite,
		...
		//do
	{
		...
		if (numberOfBitsUsedMod8 == 0)
			...
			*(data + (numberOfBitsUsed >> 3)) |= dataByte >> (numberOfBitsUsedMod8); // First half
			...
			{
				*(data + (numberOfBitsUsed >> 3) + 1) = (unsigned char) (dataByte
						<< (8 - (numberOfBitsUsedMod8))); // Second half (overlaps byte boundary)
			}
		}
...
bool BitStream::ReadBits(unsigned char* output, int numberOfBitsToRead, const bool alignBitsToRight)
{
	...
	while (numberOfBitsToRead > 0) {
		*(output + offset) |= *(data + (readOffset >> 3)) << (readOffsetMod8); // First half
		...
		if (readOffsetMod8 > 0 && numberOfBitsToRead > 8
				- (readOffsetMod8)) {   // If we have a second half, we didn't read enough bytes in the first half
			*(output + offset) |= *(data + (readOffset >> 3) + 1)
					>> (8 - (readOffsetMod8));
