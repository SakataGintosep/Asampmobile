...
public:
	...
	DataStructures::OrderedList<range_type,
								RangeNode<range_type>,
								RangeNodeComp<range_type>> ranges;
};
...
void RangeList<range_type>::Insert(range_type index)
{
	...
	if (index < ranges[insertionIndex].minIndex - 1) {
		...
		if (insertionIndex > 0
				&& ranges[insertionIndex - 1].maxIndex + 1 == ranges[insertionIndex].minIndex) {
		...
		if (insertionIndex < ranges.Size() - 1
				&& ranges[insertionIndex + 1].minIndex == ranges[insertionIndex].maxIndex + 1) {
