...
protected:
	...
	Page<KeyType, DataType, order>* InsertBranchDown(const KeyType key, const DataType& data,
			Page<KeyType, DataType, order>* cur, ReturnAction* returnAction, bool* success);
	...
	bool FindDeleteRebalance(const KeyType key, Page<KeyType, DataType, order>* cur,
			bool* underflow, KeyType rightRootKey, ReturnAction* returnAction, DataType& out);
...
template<class KeyType, class DataType, int order>
void BPlusTree<KeyType, DataType, order>::DeleteFromPageAtIndex(const int index,
...
bool BPlusTree<KeyType, DataType, order>::FixUnderflow(int branchIndex,
		Page<KeyType, DataType, order>* cur, KeyType rightRootKey, ReturnAction* returnAction)
{
...
bool BPlusTree<KeyType, DataType, order>::Insert(const KeyType key, const DataType& data)
{
	if (root == 0) {
		...
		ReturnAction returnAction;
		returnAction.action = ReturnAction::NO_ACTION;
		Page<KeyType, DataType, order>* newPage = InsertBranchDown(key, data, root, &returnAction,
				&success);
		if (success == false) {
			return false;
		}
...
bool BPlusTree<KeyType, DataType, order>::GetIndexOf(const KeyType key,
		Page<KeyType, DataType, order>* page, int* out) const
{
	assert(page->size > 0);
