...
public:
	...
	virtual bool Connect(const char* host, unsigned short serverPort, unsigned short clientPort,
			unsigned int depreciated, int threadSleepTimer) = 0;
	...
	virtual bool Send(const char* data, const int length, PacketPriority priority,
			PacketReliability reliability, char orderingChannel) = 0;
	...
	virtual bool RPC(int* uniqueID, RakNet::BitStream* bitStream, PacketPriority priority,
			...
			NetworkID networkID, RakNet::BitStream* replyFromTarget) = 0;
