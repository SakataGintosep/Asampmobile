...
#endif
bool RakClient::Connect(const char* host, unsigned short serverPort, unsigned short clientPort,
		unsigned int depreciated, int threadSleepTimer)
{
	...
	return RakPeer::Connect(host, serverPort, (char*) password.GetData(),
...
bool RakClient::Send(const char* data, const int length, PacketPriority priority,
		PacketReliability reliability, char orderingChannel)
{
	...
	return RakPeer::Send(data, length, priority, reliability, orderingChannel,
			remoteSystemList[0].playerId, false);
}

bool RakClient::Send(RakNet::BitStream* bitStream, PacketPriority priority,
		PacketReliability reliability, char orderingChannel)
{
	if (remoteSystemList == 0) {
		return false;
	...
	return RakPeer::Send(bitStream, priority, reliability, orderingChannel,
			remoteSystemList[0].playerId, false);
}
...
//			cas.Deserialize(bitStream);
			...
			inBitStream.IgnoreBits(
					8 * sizeof(unsigned int)); //inBitStream.Read(externalID.binaryAddress);
			inBitStream.IgnoreBits(
					8 * sizeof(unsigned short)); //inBitStream.Read(externalID.port);
			...
			if (i >= 0) {
				...
				otherClients[i].staticData.Write(
						(char*) bitStream.GetData() + BITS_TO_BYTES(bitStream.GetReadOffset()),
						bitStream.GetNumberOfBytesUsed()
								- BITS_TO_BYTES(bitStream.GetReadOffset()));
			}
		}
...
void RakClient::PingServer(const char* host, unsigned short serverPort, unsigned short clientPort,
...
void RakClient::RegisterAsRemoteProcedureCall(int* uniqueID,
...
bool RakClient::RPC(int* uniqueID, const char* data, unsigned int bitLength,
...
bool RakClient::RPC(int* uniqueID, RakNet::BitStream* parameters, PacketPriority priority,
...
void RakClient::AdvertiseSystem(const char* host, unsigned short remotePort, const char* data,
		int dataLength)
{
...
void RakClient::ApplyNetworkSimulator(double maxSendBPS, unsigned short minExtraPing,
