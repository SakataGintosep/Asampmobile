...
int RAK_DLL_EXPORT PlayerIDAndIndexComp(const PlayerID& key,
		const PlayerIDAndIndex& data); // GCC requires RakPeer::PlayerIDAndIndex or it won't compile
...
public:
	...
	bool Initialize(unsigned short maxConnections, unsigned short localPort, int _threadSleepTimer,
	...
	void InitializeSecurity(const char* pubKeyE, const char* pubKeyN, const char* privKeyP,
			const char* privKeyQ);
	...
	bool Connect(const char* host, unsigned short remotePort, char* passwordData,
	...
	bool Send(const char* data, const int length, PacketPriority priority,
	...
	/// \param[in] broadcast True to send this packet to all connected systems. If true, then playerId specifies who not to send the packet to.
	/// \return False if we are not connected to the specified recipient.  True otherwise
	bool Send(RakNet::BitStream* bitStream, PacketPriority priority, PacketReliability reliability,
	...
	void RegisterAsRemoteProcedureCall(int* uniqueID,
	...
	/// \param[in] replyFromTarget If 0, this function is non-blocking.  Otherwise it will block while waiting for a reply from the target procedure, which should be remotely written to RPCParameters::replyToSender and copied to replyFromTarget.  The block will return early on disconnect or if the sent packet is unreliable and more than 3X the ping has elapsed.
	/// \return True on a successful packet send (this does not indicate the recipient performed the call), false on failure
	bool RPC(int* uniqueID, RakNet::BitStream* bitStream, PacketPriority priority,
			PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast,
			bool shiftTimestamp, NetworkID networkID, RakNet::BitStream* replyFromTarget);
	...
	void CloseConnection(const PlayerID target, bool sendDisconnectionNotification,
	...
	void AdvertiseSystem(const char* host, unsigned short remotePort, const char* data,
			int dataLength);
	...
	void ApplyNetworkSimulator(double maxSendBPS, unsigned short minExtraPing,
...
protected:
	...
	friend void ProcessPortUnreachable(const unsigned int binaryAddress, const unsigned short port,
			RakPeer* rakPeer);
	friend void ProcessNetworkPacket(const unsigned int binaryAddress, const unsigned short port,
	...
	bool SendConnectionRequest(const char* host, unsigned short remotePort, char* passwordData,
			int passwordDataLength);
	///Get the reliability layer associated with a playerID.  
	/// \param[in] playerID The player identifier 
	/// \return 0 if none
	RemoteSystemStruct* GetRemoteSystemFromPlayerID(const PlayerID playerID,
			bool calledFromNetworkThread, bool onlyActive) const;
	///Parse out a connection request packet
	void ParseConnectionRequestPacket(RakPeer::RemoteSystemStruct* remoteSystem, PlayerID playerId,
			const char* data, int byteSize);
	///When we get a connection request from an ip / port, accept it unless full
	void OnConnectionRequest(RakPeer::RemoteSystemStruct* remoteSystem, unsigned char* AESKey,
			bool setAESKey);
	...
	RemoteSystemStruct* AssignPlayerIDToRemoteSystemList(const PlayerID playerId,
	...
	DataStructures::OrderedList<PlayerID,
								PlayerIDAndIndex,
								PlayerIDAndIndexComp> remoteSystemLookup;
	...
	SimpleMutex incomingQueueMutex, banListMutexTemp; //,synchronizedMemoryQueueMutex, automaticVariableSynchronizationMutex;
	...
	void CloseConnectionInternal(const PlayerID target, bool sendDisconnectionNotification,
			bool performImmediate, unsigned char orderingChannel);
	void SendBuffered(const char* data, int numberOfBitsToSend, PacketPriority priority,
			PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast,
			RemoteSystemStruct::ConnectMode connectionMode);
	bool SendImmediate(char* data, int numberOfBitsToSend, PacketPriority priority,
			PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast,
