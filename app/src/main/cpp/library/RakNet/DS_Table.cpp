...
Table::Row* Table::AddRow(unsigned rowId, DataStructures::List<Cell>& initialCellValues)
{
	...
	for (rowIndex = 0; rowIndex < columns.Size(); rowIndex++) {
		if (rowIndex < initialCellValues.Size() && initialCellValues[rowIndex].isEmpty == false) {
			newRow->cells.Insert(
					new Table::Cell(initialCellValues[rowIndex].i, initialCellValues[rowIndex].c,
...
void Table::RemoveRows(Table* tableContainingRowIDs)
{
	...
	DataStructures::Page<unsigned,
						 Row*,
						 _TABLE_BPLUS_TREE_ORDER>* cur = tableContainingRowIDs->GetRows().GetListHead();
	while (cur) {
...
void Table::QueryTable(unsigned* columnSubset, unsigned numColumnSubset,
		...
		unsigned numRowIDs, Table* result)
{
	...
	if (inclusionFilters && numInclusionFilters > 0) {
		for (i = 0; i < numInclusionFilters; i++) {
			if (inclusionFilters[i].columnIndex >= 0
					&& inclusionFilters[i].columnIndex < columns.Size())
			...
			if (row->cells[columnIndex]->isEmpty == false && columnIndex != (unsigned) -1) {
				switch (inclusionFilters[j].operation) {
					...
					case QF_LESS_THAN:
						...
						break;
					case QF_IS_EMPTY: pass = false;
						break;
					case QF_NOT_EMPTY: pass = true;
						break;
				}
			}
			else {
		...
		Table::Row* const& second) // first is the one inserting, second is the one already there.
{
	...
	for (i = 0; i < _numSortQueries; i++) {
		...
		if (first->cells[columnIndex]->isEmpty == true
			...
			return 1;
		} // Empty cells always go at the end
...
void Table::SortTable(Table::SortQuery* sortQueries, unsigned numSortQueries, Table::Row** out)
{
	...
	for (i = 0; i < numSortQueries; i++) {
		if (sortQueries[i].columnIndex >= 0 && sortQueries[i].columnIndex < columns.Size()
				&& columns[sortQueries[i].columnIndex].columnType != BINARY) {
			columnIndices.Insert(sortQueries[i].columnIndex);
			anyValid = true;
		}
		else {
			columnIndices.Insert((unsigned) -1);
		} // Means don't check this column
	}
...
DataStructures::Page<unsigned,
					 ...
					 _TABLE_BPLUS_TREE_ORDER>* Table::GetListHead(void)
{
