...
public:
	...
	virtual bool Start(unsigned short AllowedPlayers, unsigned int depreciated,
			int threadSleepTimer, unsigned short port, const char* forceHostAddress = 0) = 0;
	...
	virtual void GetPlayerIPFromID(const PlayerID playerId, char returnValue[22],
	...
	/// \param[in] data a block of data to store, or 0 for none
	/// \param[in] length The length of data in bytes, or 0 for none
	virtual void SetStaticClientData(const PlayerID playerId, const char* data,
			const int length) = 0;
	...
	/// \pre The server must be active for this to have meaning
	virtual void ChangeStaticClientData(const PlayerID playerChangedId,
			PlayerID playerToSendToId) = 0;
