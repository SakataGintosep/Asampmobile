...
public:
	...
	virtual ReplicaReturnResult SendConstruction(RakNetTime currentTime, PlayerID playerId,
			RakNet::BitStream* outBitStream, bool* includeTimestamp) = 0;
	...
	/// \return See ReplicaReturnResult
	virtual ReplicaReturnResult ReceiveDestruction(RakNet::BitStream* inBitStream,
			PlayerID playerId) = 0;
	...
	/// \param[in] playerId The participant to send to.
	/// \return See ReplicaReturnResult
	virtual ReplicaReturnResult SendScopeChange(bool inScope, RakNet::BitStream* outBitStream,
			RakNetTime currentTime, PlayerID playerId) = 0;
	...
	/// \param[in] playerId The participant that sent this message to us.
	/// \return See ReplicaReturnResult
	virtual ReplicaReturnResult ReceiveScopeChange(RakNet::BitStream* inBitStream,
			PlayerID playerId) = 0;
	...
	/// \return See ReplicaReturnResult
	virtual ReplicaReturnResult Serialize(bool* sendTimestamp, RakNet::BitStream* outBitStream,
			RakNetTime lastSendTime, PacketPriority* priority, PacketReliability* reliability,
			RakNetTime currentTime, PlayerID playerId) = 0;
	...
	/// \param[in] playerId The participant that sent this message to us.
	virtual ReplicaReturnResult Deserialize(RakNet::BitStream* inBitStream, RakNetTime timestamp,
			RakNetTime lastDeserializeTime, PlayerID playerId) = 0;
};
