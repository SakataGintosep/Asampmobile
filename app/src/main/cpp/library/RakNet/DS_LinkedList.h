...
public:
	...
	CircularLinkedListType& Add(
			const CircularLinkedListType& input); // Adds after the current position
...
bool CircularLinkedList<CircularLinkedListType>::IsIn(const CircularLinkedListType& input)
{
	...
	if (return_value != 0) {
		...
		return false;
	} // Can't find the item don't do anything
}
...
void CircularLinkedList<CircularLinkedListType>::Clear(void)
{
	...
	else if (this->list_size == 1)  // {delete root->item; delete root;}
	{
		delete this->root;
	}
