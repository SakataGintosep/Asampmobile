...
public:
	...
	/// \param[in] rightAlignedBits if true data will be right aligned
	void WriteBits(const unsigned char* input, int numberOfBitsToWrite,
			const bool rightAlignedBits = true);
...
inline bool BitStream::SerializeBits(bool writeToBitstream, unsigned char* input,
		int numberOfBitsToSerialize, const bool rightAlignedBits)
{
	if (writeToBitstream) {
...
inline bool BitStream::Read(bool& var)
{
	...
	if (data[readOffset >> 3]
			& (0x80 >> (readOffset++ % 8))) {   // Is it faster to just write it out here?
		var = true;
	}
	else {
		var = false;
