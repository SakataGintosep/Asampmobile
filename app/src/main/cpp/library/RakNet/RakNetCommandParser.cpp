...
RakNetCommandParser::RakNetCommandParser()
{
	...
	RegisterCommand(4, "CloseConnection",
	...
	RegisterCommand(3, "ApplyNetworkSimulator",
...
bool RakNetCommandParser::OnCommand(const char* command, unsigned numParameters,
		char** parameterList, TransportInterface* transport, PlayerID playerId,
		const char* originalString)
{
	...
	if (strcmp(command, "Initialize") == 0) {
		ReturnResult(peer->Initialize((unsigned short) atoi(parameterList[0]),
						(unsigned short) atoi(parameterList[1]), atoi(parameterList[2]), parameterList[3]),
		...
		ReturnResult(peer->Connect(parameterList[0], (unsigned short) atoi(parameterList[1]),
				parameterList[2], atoi(parameterList[3])), command, transport, playerId);
	}
			...
			if (count == 0) {
				...
				for (i = 0; i < count; i++)
					transport->Send(playerId, "%i %s %i:%i\r\n", i,
							peer->PlayerIDToDottedIP(remoteSystems[i]),
							remoteSystems[i].binaryAddress, remoteSystems[i].port);
			}
		}
		else {
		...
		peer->CloseConnection(IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1])),
				atoi(parameterList[2]) != 0, (unsigned char) atoi(parameterList[3]));
		ReturnResult(command, transport, playerId);
	}
	else if (strcmp(command, "GetIndexFromPlayerID") == 0) {
		ReturnResult(peer->GetIndexFromPlayerID(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
	else if (strcmp(command, "GetPlayerIDFromIndex") == 0) {
		ReturnResult(peer->GetPlayerIDFromIndex(atoi(parameterList[0])), command, transport,
				playerId);
	}
		...
		ReturnResult(peer->GetAveragePing(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
	else if (strcmp(command, "GetLastPing") == 0) {
		ReturnResult(peer->GetLastPing(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
	else if (strcmp(command, "GetLowestPing") == 0) {
		ReturnResult(peer->GetLowestPing(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
		...
		ReturnResult(peer->GetExternalID(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
	else if (strcmp(command, "SetTimeoutTime") == 0) {
		peer->SetTimeoutTime(atoi(parameterList[0]),
				IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1])));
		ReturnResult(command, transport, playerId);
	}
		...
		ReturnResult((char*) peer->PlayerIDToDottedIP(
						IntegersToPlayerID(atoi(parameterList[0]), atoi(parameterList[1]))), command,
				transport, playerId);
	}
		...
		peer->ApplyNetworkSimulator(atof(parameterList[0]), (unsigned short) atoi(parameterList[1]),
				(unsigned short) atoi(parameterList[2]));
		ReturnResult(command, transport, playerId);
	}
