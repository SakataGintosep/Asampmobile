...
struct SplitPacketChannel {
	...
	DataStructures::OrderedList<SplitPacketIndexType,
								InternalPacket*,
								SplitPacketIndexComp> splitPacketList;
};
...
public:
	...
	bool HandleSocketReceiveFromConnectedPlayer(const char* buffer, int length, PlayerID playerId,
			DataStructures::List<PluginInterface*>& messageHandlerList, int MTUSize);
	...
	bool Send(char* data, int numberOfBitsToSend, PacketPriority priority,
	...
	/// \param[in] messageHandlerList A list of registered plugins
	void Update(SOCKET s, PlayerID playerId, int MTUSize, RakNetTimeNS time,
	...
	void ApplyNetworkSimulator(double _maxSendBPS, RakNetTime _minExtraPing,
...
private:
	...
	unsigned GenerateDatagram(RakNet::BitStream* output, int MTUSize, bool* reliableDataSent,
			RakNetTimeNS time, PlayerID playerId,
			DataStructures::List<PluginInterface*>& messageHandlerList);
	...
	int WriteToBitStreamFromInternalPacket(RakNet::BitStream* bitStream,
	...
	InternalPacket* BuildPacketFromSplitPacketList(SplitPacketIdType splitPacketId,
			RakNetTimeNS time);
	...
	InternalPacket* CreateInternalPacketCopy(InternalPacket* original, int dataByteOffset,
			int dataByteLength, RakNetTimeNS time);

	/// Get the specified ordering list
	DataStructures::LinkedList<InternalPacket*>* GetOrderingListAtOrderingStream(
	...
	void InsertPacketIntoResendList(InternalPacket* internalPacket, RakNetTimeNS time,
	...
	DataStructures::OrderedList<SplitPacketIdType,
								SplitPacketChannel*,
