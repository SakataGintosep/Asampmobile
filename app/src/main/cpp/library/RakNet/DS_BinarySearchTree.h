...
void AVLBalancedBinarySearchTree<BinarySearchTreeType>::Add(const BinarySearchTreeType& input)
{
	...
	typename BinarySearchTree<BinarySearchTreeType>::node* current = BinarySearchTree<
			BinarySearchTreeType>::Add(input);
...
{
	typename BinarySearchTree<BinarySearchTreeType>::node* current = BinarySearchTree<
			BinarySearchTreeType>::Del(input);
		...
		typename BinarySearchTree<BinarySearchTreeType>::node* C)
{
	...
	*/
	...
	if (A) {
		...
		this->root = C;
	}  // If B has no parent parent then B must have been the root node
	...
	while (1) {
		if (((current->left) != 0) && (*(current->item) < *(current->left->item))) {
			...
			*(current->item) = temp;
			...
			break;
		}  // current points to the right place so quit
	}
}
		...
		const BinarySearchTreeType& input)
{
	...
	else if ((current->right) != 0 && (current->left)
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		while (true)    // This loop traverses the tree to find a spot for insertion
		{
			...
			if (input < *(current->item)) {
				...
				return 0;
			} // ((input == current->item) == true) which is not allowed since the tree only takes discrete values.  Do nothing
		}
	}
}
