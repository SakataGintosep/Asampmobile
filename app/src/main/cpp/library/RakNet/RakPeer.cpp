...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Initialize(unsigned short maxConnections, unsigned short localPort,
		int _threadSleepTimer, const char* forceHostAddress)
{
	if (IsActive()) {
		return false;
	}
...
#ifndef _RELEASE
			remoteSystemList[i].reliabilityLayer.ApplyNetworkSimulator(_maxSendBPS, _minExtraPing,
					_extraPingVariance);
#endif
		}
	...
	if (endThreads) {
...
#endif
		{
			...
			if (isMainLoopThreadActive == false) {
				error = pthread_create(&processPacketsThreadHandle, &attr, &UpdateNetworkLoop,
						this);
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::InitializeSecurity(const char* pubKeyE, const char* pubKeyN, const char* privKeyP,
		const char* privKeyQ)
{
#if !defined(_COMPATIBILITY_1)
	if (endThreads == false) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Connect(const char* host, unsigned short remotePort, char* passwordData,
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::Send(const char* data, const int length, PacketPriority priority,
		PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast)
{
	...
	if (broadcast == false && router && GetIndexFromPlayerID(playerId) == -1) {
		return router->Send(data, BYTES_TO_BITS(length), priority, reliability, orderingChannel,
				playerId);
	}
	else {
...
bool RakPeer::Send(RakNet::BitStream* bitStream, PacketPriority priority,
		PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast)
{
#ifdef _DEBUG
	...
	if (broadcast == false && router && GetIndexFromPlayerID(playerId) == -1) {
		return router->Send((const char*) bitStream->GetData(), bitStream->GetNumberOfBitsUsed(),
		...
		SendBuffered((const char*) bitStream->GetData(), bitStream->GetNumberOfBitsUsed(), priority,
				reliability, orderingChannel, playerId, broadcast, RemoteSystemStruct::NO_ACTION);
	}
...
Packet* RakPeer::Receive(void)
{
	...
	while (packet && (packet->data[0] == ID_RPC
			|| (packet->length > sizeof(unsigned char) + sizeof(RakNetTime)
					&& packet->data[0] == ID_TIMESTAMP
					&& packet->data[sizeof(unsigned char) + sizeof(RakNetTime)] == ID_RPC))) {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::RegisterAsRemoteProcedureCall(int* uniqueID,
...
//	char *writeTarget; // Used to hold either a block of allocated data or the externally allocated data
	...
	for (sendListIndex = 0; sendListIndex < (unsigned) sendListSize; sendListIndex++) {
		...
		if (bitLength > 0) {
			outgoingBitStream.WriteBits((const unsigned char*) data, bitLength,
					false); // Last param is false to write the raw data originally from another bitstream, rather than shifting from user data
		}
		else {
			outgoingBitStream.WriteCompressed((unsigned int) 0);
		}

		Send(&outgoingBitStream, priority, reliability, orderingChannel,
				remoteSystemList[sendList[sendListIndex]].playerId, false);
	...
	if (replyFromTarget) {
		...
		if (reliability == UNRELIABLE) {
			if (playerId == UNASSIGNED_PLAYER_ID) {
				stopWaitingTime = RakNet::GetTime()
						+ 1500; // Lets guess the ave. ping is 500.  Not important to be very accurate
			}
			else {
		...
		while (blockOnRPCReply &&
				((reliability == RELIABLE || reliability == RELIABLE_ORDERED
...
#pragma warning( disable : 4701 ) // warning C4701: local variable <variable name> may be used without having been initialized
#endif
bool RakPeer::RPC(int* uniqueID, RakNet::BitStream* bitStream, PacketPriority priority,
		PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast,
		bool shiftTimestamp, NetworkID networkID, RakNet::BitStream* replyFromTarget)
{
	if (bitStream) {
		return RPC(uniqueID, (const char*) bitStream->GetData(), bitStream->GetNumberOfBitsUsed(),
				priority, reliability, orderingChannel, playerId, broadcast, shiftTimestamp,
				networkID, replyFromTarget);
	}
	else {
		return RPC(uniqueID, 0, 0, priority, reliability, orderingChannel, playerId, broadcast,
...
PlayerID RakPeer::GetPlayerIDFromIndex(int index)
{
	...
	if (index >= 0 && index < maximumNumberOfPeers) {
		if (remoteSystemList[index].connectMode
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::Ping(const char* host, unsigned short remotePort,
		bool onlyReplyOnAcceptingConnections)
{
	...
	unsigned i;
	for (i = 0; i < messageHandlerList.Size(); i++)
		messageHandlerList[i]->OnDirectSocketSend((const char*) bitStream.GetData(),
				bitStream.GetNumberOfBitsUsed(), playerId);
	// No timestamp for 255.255.255.255
	SocketLayer::Instance()->SendTo(connectionSocket, (const char*) bitStream.GetData(),
			bitStream.GetNumberOfBytesUsed(), (char*) host, remotePort);
...
int RakPeer::GetLastPing(const PlayerID playerId) const
{
	...
	if (remoteSystem->pingAndClockDifferentialWriteIndex == 0) {
		...
		return remoteSystem->pingAndClockDifferential[
				remoteSystem->pingAndClockDifferentialWriteIndex - 1].pingTime;
	}
}
...
// remotePort: Which port to connect to on the remote machine.
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::AdvertiseSystem(const char* host, unsigned short remotePort, const char* data,
		int dataLength)
{
	if (IsActive() == false) {
		return;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::ApplyNetworkSimulator(double maxSendBPS, unsigned short minExtraPing,
...
void RakPeer::ParseConnectionRequestPacket(RakPeer::RemoteSystemStruct* remoteSystem,
		PlayerID playerId, const char* data, int byteSize)
{
	...
	if (!AllowIncomingConnections()) {
		...
		SendImmediate((char*) &c, sizeof(char) * 8, SYSTEM_PRIORITY, RELIABLE, 0, playerId, false,
		...
		if (incomingPasswordLength == passwordLength &&
			...
			// SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, ( char* ) & c, sizeof( char ), systemAddress.binaryAddress, systemAddress.port );
			SendImmediate((char*) &c, sizeof(char) * 8, SYSTEM_PRIORITY, RELIABLE, 0, playerId,
					false, false, RakNet::GetTime());
			remoteSystem->connectMode = RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY;
		}
	}
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::OnConnectionRequest(RakPeer::RemoteSystemStruct* remoteSystem, unsigned char* AESKey,
		bool setAESKey)
{
	...
	//if ( AllowIncomingConnections() )
	{
		...
		RakNet::BitStream bitStream(
				sizeof(unsigned char) + sizeof(unsigned short) + sizeof(unsigned int)
						+ sizeof(unsigned short) + sizeof(PlayerIndex));
		...
		SendImmediate((char*) bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY,
		...
		unsigned char orderingChannel)
{
	...
	if (performImmediate) {
		SendImmediate((char*) temp.GetData(), temp.GetNumberOfBitsUsed(), LOW_PRIORITY,
				RELIABLE_ORDERED, orderingChannel, playerId, false, false, RakNet::GetTime());
		RemoteSystemStruct* rss = GetRemoteSystemFromPlayerID(playerId, true, true);
		rss->connectMode = RemoteSystemStruct::DISCONNECT_ASAP;
	}
	else {
		SendBuffered((const char*) temp.GetData(), temp.GetNumberOfBitsUsed(), LOW_PRIORITY,
				RELIABLE_ORDERED, orderingChannel, playerId, false,
				RemoteSystemStruct::DISCONNECT_ASAP);
	}
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned short RakPeer::GetNumberOfRemoteInitiatedConnections(void) const
{
	...
	for (i = 0; i < maximumNumberOfPeers; i++)
		//for ( i = 0; i < remoteSystemListSize; i++ )
	{
		if (remoteSystemList[i].isActive && remoteSystemList[i].weInitiatedTheConnection == false
				&& remoteSystemList[i].connectMode == RemoteSystemStruct::CONNECTED) {
...
bool RakPeer::HandleRPCPacket(const char* data, int length, PlayerID playerId)
{
	...
	if (rpcParms.numberOfBitsOfData == 0) {
...
#if !defined(_COMPATIBILITY_1)
		if (BITS_TO_BYTES(incomingBitStream.GetNumberOfUnreadBits())
				< MAX_ALLOCA_STACK_ALLOCATION) {
			userData = (unsigned char*) alloca(
					BITS_TO_BYTES(incomingBitStream.GetNumberOfUnreadBits()));
			usedAlloca = true;
		}
		else
#endif
		...
		if (incomingBitStream.ReadBits((unsigned char*) userData, rpcParms.numberOfBitsOfData,
				false) == false) {
#ifdef _DEBUG
			assert(0);
#endif
...
void RakPeer::SecuredConnectionResponse(const PlayerID playerId)
{
	...
	SendImmediate((char*) connectionRequestResponse,
			(1 + sizeof(big::u32) + sizeof(RSA_BIT_SIZE) + 20) * 8, SYSTEM_PRIORITY, UNRELIABLE, 0,
...
void RakPeer::SecuredConnectionConfirmation(RakPeer::RemoteSystemStruct* remoteSystem, char* data)
{
	...
	*/
	...
	memcpy(reply + 1 + 20, encryptedMessage,
			sizeof(RSA_BIT_SIZE)); // Copy the encoded random number

	//SocketLayer::Instance()->SendTo( connectionSocket, reply, 1 + 20 + sizeof( RSA_BIT_SIZE ), playerId.binaryAddress, playerId.port );
	// All secure connection requests are unreliable because the entire process needs to be restarted if any part fails.
	// Connection requests are resent periodically
	SendImmediate((char*) reply, (1 + 20 + sizeof(RSA_BIT_SIZE)) * 8, SYSTEM_PRIORITY, UNRELIABLE,
			0, remoteSystem->playerId, false, false, RakNet::GetTime());
	//	}
...
void RakPeer::SendStaticDataInternal(const PlayerID target, bool performImmediate)
{
	...
	if (performImmediate) {
		if (target == UNASSIGNED_PLAYER_ID) {
			SendImmediate((char*) reply.GetData(), reply.GetNumberOfBitsUsed(), SYSTEM_PRIORITY,
					RELIABLE, 0, target, true, false, RakNet::GetTime());
		}
		else {
			SendImmediate((char*) reply.GetData(), reply.GetNumberOfBitsUsed(), SYSTEM_PRIORITY,
					RELIABLE, 0, target, false, false, RakNet::GetTime());
		}
	}
	else {
		if (target == UNASSIGNED_PLAYER_ID) {
...
void RakPeer::PingInternal(const PlayerID target, bool performImmediate)
{
	...
	if (performImmediate) {
		SendImmediate((char*) bitStream.GetData(), bitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY,
				UNRELIABLE, 0, target, false, false, currentTimeNS);
	}
	else {
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void RakPeer::CloseConnectionInternal(const PlayerID target, bool sendDisconnectionNotification,
		...
		//for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; remoteSystemIndex++ )
	{
		if (remoteSystemList[remoteSystemIndex].isActive &&
				...
				&& // Not fully connected players are not valid user-send targets because the reliability layer wasn't reset yet
						((broadcast == false
								&& remoteSystemList[remoteSystemIndex].playerId == playerId) ||
								(broadcast == true && remoteSystemList[remoteSystemIndex].playerId
										!= playerId))
				) {
			return true;
		}
...
#endif
	bcs = bufferedCommands.WriteLock();
	bcs->data = new char[BITS_TO_BYTES(
			numberOfBitsToSend)]; // Making a copy doesn't lose efficiency because I tell the reliability layer to use this allocation for its own copy
#ifdef _DEBUG
	assert(bcs->data);
#endif
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
bool RakPeer::SendImmediate(char* data, int numberOfBitsToSend, PacketPriority priority,
		PacketReliability reliability, char orderingChannel, PlayerID playerId, bool broadcast,
		bool useCallerDataAllocation, RakNetTimeNS currentTime)
{
	unsigned* sendList;
	unsigned sendListSize;
	...
	for (sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++) {
		...
		if (outputTree) {
			...
			remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(
					(char*) bitStreamCopy.GetData(), bitStreamCopy.GetNumberOfBitsUsed(), priority,
					reliability, orderingChannel, true, MTUSize, currentTime);
		}
		else {
			...
			remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(data,
					numberOfBitsToSend, priority, reliability, orderingChannel, useData == false,
					MTUSize, currentTime);
			if (useData) {
		...
		if (reliability == RELIABLE || reliability == RELIABLE_ORDERED
				|| reliability == RELIABLE_SEQUENCED) {
			remoteSystemList[sendList[sendListIndex]].lastReliableSend = (RakNetTime) (currentTime
					/ (RakNetTimeNS) 1000);
		}
	}
...
#else
void ProcessNetworkPacket(const unsigned int binaryAddress, const unsigned short port,
...
#endif
{
	...
	if (rakPeer->IsBanned(rakPeer->PlayerIDToDottedIP(playerId))) {
		...
		SocketLayer::Instance()->SendTo(rakPeer->connectionSocket, (char*) &c, 2,
	...
	if ((unsigned char) (data)[0] == (unsigned char) ID_CONNECTION_BANNED
	...
	if ((unsigned char) (data)[0] == (unsigned char) ID_NO_FREE_INCOMING_CONNECTIONS
			&& length <= sizeof(unsigned char) * 2) {
		// Remove the connection attempt from the buffered commands
		RakPeer::RequestedConnectionStruct* rcsFirst, * rcs;
		rcsFirst = rakPeer->requestedConnectionList.ReadLock();
		...
		//
	else if ((unsigned char) (data)[0] == (unsigned char) ID_OPEN_CONNECTION_REPLY
			&& length <= sizeof(unsigned char) * 2) {
		for (i = 0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length * 8, playerId);
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
		...
		while (rcs) {
			...
			if (rcs->playerId == playerId) {
				...
				if (remoteSystem == 0) {
					remoteSystem = rakPeer->AssignPlayerIDToRemoteSystemList(playerId,
				...
				if (remoteSystem) {
					...
					rakPeer->SendImmediate((char*) temp.GetData(), temp.GetNumberOfBitsUsed(),
							SYSTEM_PRIORITY, RELIABLE, 0, playerId, false, false, time);
				}
		...
		*/
	...
	else if ((unsigned char) (data)[0] == (unsigned char) ID_CONNECTION_ATTEMPT_FAILED
			&& length <= sizeof(unsigned char) * 2) {
		// Remove the connection attempt from the buffered commands
		RakPeer::RequestedConnectionStruct* rcsFirst, * rcs;
		rcsFirst = rakPeer->requestedConnectionList.ReadLock();
		...
		// We didn't check this datagram to see if it came from a connected system or not yet.
		// Therefore, this datagram must be under 17 bits - otherwise it may be normal network traffic as the min size for a raknet send is 17 bits
	else if ((unsigned char) (data)[0] == ID_OPEN_CONNECTION_REQUEST
			&& length == sizeof(unsigned char) * 2 + 1) // 0.3d RAKSAMP HACK PONPON
	{
		for (i = 0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length * 8, playerId);
		...
		if (rss == 0 || rss->weInitiatedTheConnection == true) {
			...
			if (rss == 0) {
				rss = rakPeer->AssignPlayerIDToRemoteSystemList(playerId,
						RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER);
			}
			...
			&& length == sizeof(unsigned char) * 2 + 1) // RAKSAMP HACK PONPON
	{
		for (i = 0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length * 8, playerId);
		...
		*(uint16_t*) &c[1] = (*(uint16_t*) &data[1] ^ NETCODE_CONNCOOKIELULZ);
		...
		SocketLayer::Instance()->SendTo(rakPeer->connectionSocket, (char*) &c, 3,
				playerId.binaryAddress, playerId.port);
		return;
	}
	...
	if (remoteSystem) {
		...
		if (remoteSystem->connectMode
				== RakPeer::RemoteSystemStruct::SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET) {
			//if ((length % 16) == 0)
		...
		if (remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(data, length,
				playerId, rakPeer->messageHandlerList, rakPeer->MTUSize) == false) {
					...
					((unsigned char) data[0] == ID_OPEN_CONNECTION_REQUEST && length <= 3) ||
							...
							||
									(((unsigned char) data[0] == ID_PING_OPEN_CONNECTIONS
											...
											|| (unsigned char) data[0] == ID_PONG)
											&& length >= sizeof(unsigned char) + sizeof(RakNetTime))
							||
									((unsigned char) data[0] == ID_ADVERTISE_SYSTEM
											&& length < MAX_OFFLINE_DATA_LENGTH)
			))
#endif
			{
		...
		if (((unsigned char) data[0] == ID_PING_OPEN_CONNECTIONS
					...
					rakPeer->AllowIncomingConnections()) // Open connections with players
			{
				...
				outBitStream.Write(
						(unsigned char) ID_PONG); // Should be named ID_UNCONNECTED_PONG eventually
				...
				unsigned i;
				for (i = 0; i < rakPeer->messageHandlerList.Size(); i++)
					rakPeer->messageHandlerList[i]->OnDirectSocketSend(
							(const char*) outBitStream.GetData(),
							outBitStream.GetNumberOfBytesUsed(), playerId);

				SocketLayer::Instance()->SendTo(rakPeer->connectionSocket,
		...
		else if ((unsigned char) data[0] == ID_PONG
				&& length >= sizeof(unsigned char) + sizeof(RakNetTime)
		...
		else if ((unsigned char) data[0] == ID_ADVERTISE_SYSTEM && length >= 2
				&& length < MAX_OFFLINE_DATA_LENGTH + 2) {
			packet = AllocPacket(length, __FILE__, __LINE__);
			memcpy(packet->data, data, length);
			packet->bitSize = length * 8;
			packet->playerId = playerId;
...
//	unsigned numberOfBytesUsed;
	...
	while ((bcs = bufferedCommands.ReadLock()) != 0) {
		if (bcs->command == BufferedCommandStruct::BCS_SEND) {
			...
			callerDataAllocationUsed = SendImmediate((char*) bcs->data, bcs->numberOfBitsToSend,
					bcs->priority, bcs->reliability, bcs->orderingChannel, bcs->playerId,
					bcs->broadcast, true, timeNS);
			...
			if (bcs->connectionMode != RemoteSystemStruct::NO_ACTION
					&& bcs->playerId != UNASSIGNED_PLAYER_ID) {
	...
	while (rcs) {
		...
		if (rcs->nextRequestTime < timeMS) {
			...
			SocketLayer::Instance()->SendTo(connectionSocket, (char*) &c, 3,
					rcs->playerId.binaryAddress, rcs->playerId.port);
		}
		...
		if (remoteSystemList[remoteSystemIndex].isActive) {
			...
			if (timeMS > remoteSystem->lastReliableSend
				...
				if (rnss->messagesOnResendQueue == 0) {
					...
					SendImmediate((char*) &keepAlive, 8, LOW_PRIORITY, RELIABLE, 0,
							remoteSystem->playerId, false, false, timeNS);
					remoteSystem->lastReliableSend =
							timeMS + remoteSystem->reliabilityLayer.GetTimeoutTime();
				}
			}

			remoteSystem->reliabilityLayer.Update(connectionSocket, playerId, MTUSize, timeNS,
					messageHandlerList); // playerId only used for the internet simulator test
			...
			if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
					...
					((
							(remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION
									...
									remoteSystem->connectMode
											== RemoteSystemStruct::SET_ENCRYPTION_ON_MULTIPLE_16_BYTE_PACKET)
			...
			while (bitSize > 0) {
				...
				if (inputTree) {
					...
					if (numberOfBitsUsed > 0) {
						...
						memcpy(dataCopy, dataBitStream.GetData(),
								dataBitStream.GetNumberOfBytesUsed());
						dataBitStream.Reset();
						...
						if (byteSize > BITS_TO_BYTES(
								bitSize))   // Probably the case - otherwise why decompress?
						{
							delete[] data;
				...
				if (remoteSystem->connectMode == RemoteSystemStruct::UNVERIFIED_SENDER) {
					if ((unsigned char) (data)[0] == ID_CONNECTION_REQUEST) {
						ParseConnectionRequestPacket(remoteSystem, playerId, (const char*) data,
								byteSize);
						delete[] data;
					}
					else {
...
#if !defined(_COMPATIBILITY_1)
						AddToBanList(PlayerIDToDottedIP(playerId),
					...
					else if ((unsigned char) data[0] == ID_NEW_INCOMING_CONNECTION && byteSize
...
//						assert(remoteSystem->connectMode==RemoteSystemStruct::HANDLING_CONNECTION_REQUEST);
										...
										playerId == myPlayerId) // local system connect
						{
							...
							&& byteSize == sizeof(unsigned char) + sizeof(RakNetTime) * 2) {
						...
						if (lastPing <= 0 || ((ping < (lastPing * 3)) && ping < 1200)) {
							...
							assert(ping
									< 10000); // Sanity check - could hit due to negative pings causing the var to overflow
					...
					else if ((unsigned char) data[0] == ID_INTERNAL_PING
							&& byteSize == sizeof(unsigned char) + sizeof(RakNetTime)) {
						RakNet::BitStream inBitStream((unsigned char*) data, byteSize, false);
						inBitStream.IgnoreBits(8);
						RakNetTime sendPingTime;
						inBitStream.Read(sendPingTime);
						...
						SendImmediate((char*) outBitStream.GetData(),
								outBitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, UNRELIABLE, 0,
								playerId, false, false, timeMS);
						...
						*/
						...
						remoteSystem->staticData.Reset();
						remoteSystem->staticData.Write((char*) data + sizeof(unsigned char),
								byteSize - 1);
						...
						// s2c syn-cookie = SHA1_HASH(source ip address + source port + random number)
						sha1.Reset();
						sha1.Update((unsigned char*) &playerId.binaryAddress,
								sizeof(playerId.binaryAddress));
						sha1.Update((unsigned char*) &playerId.port, sizeof(playerId.port));
						sha1.Update((unsigned char*) &(newRandomNumber), 20);
						sha1.Final();
					...
					else if ((unsigned char) (data)[0] == ID_CONNECTION_REQUEST_ACCEPTED && byteSize
							== sizeof(unsigned char) + sizeof(unsigned int) + sizeof(unsigned short)
									+ sizeof(PlayerIndex)
											/* RAKSAMP HACK HACK HACK*/ + sizeof(unsigned short)
									+ sizeof(unsigned short)) {
						...
						if (allowConnection) {
							...
							if (alreadyConnected == false) {
								...
								if (remoteSystem->setAESKey) {
									remoteSystem->reliabilityLayer.SetEncryptionKey(
											remoteSystem->AESKey);
								}
								else {
									remoteSystem->reliabilityLayer.SetEncryptionKey(0);
							...
							SendImmediate((char*) outBitStream.GetData(),
									outBitStream.GetNumberOfBitsUsed(), SYSTEM_PRIORITY, RELIABLE,
									0, playerId, false, false, RakNet::GetTime());
					...
					else if (byteSize > (sizeof(unsigned char) + sizeof(unsigned char))
							&& (unsigned char) (data)[0] == ID_AUTH_KEY) {
						packet = AllocPacket(byteSize, data, __FILE__, __LINE__);
						packet->bitSize = bitSize;
						packet->playerId = playerId;
						packet->playerIndex = (PlayerIndex) remoteSystemIndex;
